...
function actingChallenge() {
    ...
    challenge.generateDescription();
    if (!teamCounter && randomNumber(0, 100) >= 50 && currentCast.length > 6 && currentCast.length <= 15 && !isTeamChallenge && (top3 || top4)){
...
function marketingChallenge() {
    ...
    challenge.generateDescription();
    if (!teamCounter && randomNumber(0, 100) >= 60 && currentCast.length > 6 && currentCast.length <= 15 && !isTeamChallenge && (top3 || top4)){
        isTeamChallenge = true;
        teamConunter = true;
        teamMaking();
        challenge.rankPerformances();
...
function danceChallenge() {
    ...
    challenge.generateDescription();
    if (!teamCounter && randomNumber(0, 100) >= 70 && currentCast.length > 6 && currentCast.length <= 15 && !isTeamChallenge && (top3 || top4)){
        isTeamChallenge = true;
        teamCounter = true;
        teamMaking();
        challenge.rankPerformances();
...
function girlgroup() {
    ...
    challenge.generateDescription();
    if (!teamCounter && randomNumber(0, 100) >= 50 && currentCast.length > 6 && currentCast.length <= 15 && !isTeamChallenge && (top3 || top4) && episodeCount > 3) {
        isTeamChallenge = true;
        teamCounter = true;
        teamMaking();
        challenge.rankPerformances();
...
function talentshow() {
    ...
    if (s14Premiere && episodeCount == 2 || (lipsync_assassin || all_stars || gde_all_stars)) {
...
function dipslayPerformanceRanking(arrayOfQueens) {
    let slay = arrayOfQueens.filter(function (queen) { return queen.performanceScore < 6; });
    let great = arrayOfQueens.filter(function (queen) { return queen.performanceScore >= 6 && queen.performanceScore < 16; });
    let good = arrayOfQueens.filter(function (queen) { return queen.performanceScore >= 16 && queen.performanceScore < 26; });
    let bad = arrayOfQueens.filter(function (queen) { return queen.performanceScore >= 26 && queen.performanceScore < 31; });
    let flop = arrayOfQueens.filter(function (queen) { return queen.performanceScore >= 31 && queen.performanceScore < 50; });
    createPerformanceDesc(slay, great, good, bad, flop);
}
function teamMaking() {
    let screen = new Scene();
...
function runway() {
    ...
    else if (currentCast.length == 3 && top3 || currentCast.length == 5 && top4 || currentCast.length == 4 && (all_stars || lipsync_assassin || gde_all_stars) || currentCast.length == 2 && team)
    ...
    else if (currentCast.length == 4 && (all_stars || lipsync_assassin || gde_all_stars))
...
function createChallenge(challenges, miniChallengeScreen) {
    ...
    else if (currentCast.length == totalCastSize && !talentShowCounter && (gde_all_stars || all_stars || lipsync_assassin) || currentCast == firstCast && s14Premiere || currentCast == secondCast && s14Premiere || all_winners && currentCast.length <= 10 && episodeCount == 11 || all_winners && currentCast.length > 10 && episodeCount == 15)
    ...
    //snatch game for -10 cast
        ...
        miniChallengeScreen.createButton("Proceed", "snatchGame()");
    //the ball for the fourth competitive episode for lsftc or all winners/gde all stars
    else if (currentCast.length == totalCastSize - 3 && (top4 || gde_all_stars) && !ballCounter || all_winners && episodeCount == 3)
    ...
    else if (currentCast.length == totalCastSize - 4 && (top4 || gde_all_stars || all_stars || lipsync_assassin) && !ballCounter || currentCast.length == 3 && team)
        miniChallengeScreen.createButton("Proceed", "ball()");
    //Girl Group
...
function createPerformanceDescWithID(slay, great, good, bad, flop, id) {
    let screen = new Scene();
    if (slay.length !== 0) {
        for (let i = 0; i < slay.length; i++)
            screen.createImage(slay[i].image, "darkblue");
        screen.createBold("", "slay" + id);
        let slayText = document.getElementById("slay" + id);
        for (let i = 0; i < slay.length; i++)
            slayText.innerHTML += `${slay[i].getName()}, `;
        slayText.innerHTML += "slayed the challenge!";
    }
    if (great.length !== 0) {
        for (let i = 0; i < great.length; i++)
            screen.createImage(great[i].image, "royalblue");
        screen.createBold("", "great" + id);
        let greatText = document.getElementById("great" + id);
        for (let i = 0; i < great.length; i++)
            greatText.innerHTML += `${great[i].getName()}, `;
        greatText.innerHTML += "had a great performance!";
    }
    if (good.length !== 0) {
        for (let i = 0; i < good.length; i++)
            screen.createImage(good[i].image);
        screen.createBold("", "good" + id);
        let goodText = document.getElementById("good" + id);
        for (let i = 0; i < good.length; i++)
            goodText.innerHTML += `${good[i].getName()}, `;
        goodText.innerHTML += "had a good performance.";
    }
    if (bad.length !== 0) {
        for (let i = 0; i < bad.length; i++)
            screen.createImage(bad[i].image, "pink");
        screen.createBold("", "bad" + id);
        let badText = document.getElementById("bad" + id);
        for (let i = 0; i < bad.length; i++)
            badText.innerHTML += `${bad[i].getName()}, `;
        badText.innerHTML += "had a bad performance...";
    }
    if (flop.length !== 0) {
        for (let i = 0; i < flop.length; i++)
            screen.createImage(flop[i].image, "tomato");
        screen.createBold("", "flop" + id);
        let flopText = document.getElementById("flop" + id);
        for (let i = 0; i < flop.length; i++)
            flopText.innerHTML += `${flop[i].getName()}, `;
        flopText.innerHTML += "flopped the challenge...";
    }
...
function newEpisode() {
    ...
    else if (currentCast.length == 4 && (gde_all_stars || all_stars || lipsync_assassin))
...
function finaleFinale() {
    ...
    if (currentCast[0].finaleScore == currentCast[1].finaleScore && randomNumber(0, 100) >= 90) {
        ...
        screen.createBold("Now prance, my queens!");
        if (!allstars3Finale && !ukvstwFinale && !top2finaleAS && (gde_all_stars || all_stars || lipsync_assassin) || isThisA3Way) {
...
function contestantProgress() {
    ...
    if (onFinale) {
        ...
        for (let i = 0; i < winnerQueen.trackRecord.length + 1; i++) {
            ...
            if (placement.innerHTML == "WIN") {
                ...
                placement.setAttribute("style", "font-weight: bold; background-color: #9f0028;");
            }
                ...
                placement.setAttribute("style", "font-weight: bold; background-color: turquoise;");
            }
            else if (placement.innerHTML == "  WIN") {
            ...
            else if (placement.innerHTML == "WIN+OUT") {
                placement.setAttribute("style", "font-weight: bold; background-color: crimson;");
            }
                ...
                placement.setAttribute("style", "background-color: #ffaea2;");
            }
    ...
    if (lipsync_assassin || all_stars || gde_all_stars) {
        var titlea = document.createElement("big");
        titlea.innerHTML = "Lip-sync For Your Legacy";
...
function predefCast(cast, format, premiere = '', returning = '') {
    ...
    else if (returning == "goldentributes")
        goldentributes = true;
    ...
    else if (goldentributes && !gde_all_stars) {
        window.alert("The Golden Tributes format isn't supported with non-saving formats, sorry!");
        s14Premiere = false;
        s12Premiere = false;
        s9Premiere = false;
        s6Premiere = false;
...
function startSimulation(challenge = "") {
    ...
    if (currentCast.length == 0)
        ...
        else if (select.options[select.selectedIndex].value == "gde-allstars") {
            gde_all_stars = true;
        }
        ...
        else if (select3.options[select3.selectedIndex].value == "goldentributes")
            goldentributes = true;
        if (document.getElementById("disableDouble").checked == true)
            noDouble = true;
        if (document.getElementById("chocolateBar").checked == true)
        ...
        else if (gde_all_stars && (lalaparuza || queensOfComedy || conjoinedQueens || kittyGirlGroup || smackdown || s6Premiere || s12Premiere || s14Premiere || s9Premiere)) {
            window.alert("The settings you've chosen are incompatible with the Lip-Sync For Your Life And Legacy format. Please select the normal premiere and Golden Tributes.")
            s14Premiere = false;
            s12Premiere = false;
            s9Premiere = false;
            s6Premiere = false;
...
function judging() {
    if ((s12Premiere || porkchopPremiere) && premiereCounter <= 2) {
        ...
        topQueens.push(currentCast[0]);
        topQueens.push(currentCast[1]);
        //TODO: Check if removing this line improves the judging like assumed
        if (/*currentCast[2].performanceScore >= 6 && */currentCast[2].performanceScore < 16 && !all_stars && !gde_all_stars)
...
function judgingScreen() {
    ...
    if (uk3Premiere && episodeCount == 1)
        ...
        judgingScreen.createButton("Proceed", "top2AndSaved()");
}
...
function winAndBtm2() {
    ...
    //sort the top queens now taking runway and favoritism in consideration:
    for (let i = 0; i < topQueens.length; i++)
        topQueens[i].performanceScore -= (topQueens[i].runwayScore - topQueens[i].favoritism * 3 / 4);
    topQueens.sort((a, b) => (a.performanceScore - b.performanceScore));
    if (isTeamChallenge) {
    ...
    //do the same, but for the bottom queens:
    for (let i = 0; i < bottomQueens.length; i++)
        bottomQueens[i].performanceScore -= (bottomQueens[i].runwayScore - bottomQueens[i].favoritism * 3 / 4);
    bottomQueens.sort((a, b) => (a.performanceScore - b.performanceScore));
...
function top2AndSaved() {
   ...
   /* if (bottomQueens.length >= 3 && currentCast.length > 5){
    ...
    let temperaments = [];
    for (let i = 0; i < top2.length; i++) {
        //decisionData stores the choice and the queen's temperament while making the choice
        let decisionData = whoToSave(top2[i], bottomQueens);
        temperaments.push(decisionData["temperament"]);
        top2[i].lipstick = decisionData["choice"];
        for (let o = 0; o < bottomQueens.length; o++) {
            if (top2[i].lipstick != bottomQueens[o]) {
                modRelation(2, 1, top2[i], bottomQueens[o]);
            }
                ...
                if (randomNumber(0, 1) == 0) {
                    modRelation(2, 3, top2[i], bottomQueens[o]);
                }
            }
        }
        screen.createImage(top2[i].image, "cyan");
        screen.createImage(top2[i].lipstick.image, "green");
        screen.createBold(top2[i].getName() + " chose to save " + top2[i].lipstick.getName(), "winV", "winVP");
        if (randomNumber(0, 1) == 0) {
            screen.createBold( top2[i].lipstick.getName() + " was very grateful.", "winV", "winVP");
        } else {
            screen.createBold( top2[i].lipstick.getName() + " was surprised, but glad nonetheless.","winV", "winVP");
        }
    }
    if (top2[0].lipstick == top2[1].lipstick){
        let imageVoted = document.querySelectorAll("img[src='" + top2[0].lipstick.image +"']")[1];
    ...
    main.appendChild(br);
    screen.createButton("Show lipsticks", "showvotes()", "showvotes");
    screen.createButton("Proceed", "gdeLipSync([" + temperaments[0] + ", " + temperaments[1] +"])");
}
let mismovoto = false;
function showSingleVote() {
...
function showvotes() {
    ...
    }else if (all_stars || gde_all_stars){
        let winV = document.querySelectorAll("p#winVP");
        for (let i = 0; i < winV.length; i++){
            winV[i].removeAttribute("hidden");
        }
...
function lipSync() {
    ...
    if (thirdqueen && currentCast.length > 5) {
        ...
        if (score1 > 7 && score2 > 7 && randomNumber(0, 100) <= 50 && !doubleShantay && noDouble == false && currentCast.length > 5) {
            ...
            screen.createBold("We've been in touch with the doctor. You need time to heal. I cannot, in good conscience, allow you to continue in the competition.");
...
function gdeLipSync(temperaments) {
    for (let i = 0; i < top2.length; i++) {
        top2[i].getASLipsync();
    }
    if (top2[1].lipsyncScore > top2[0].lipsyncScore) {
        //reverse the temperaments array if needed
        let temp = temperaments[0];
        temperaments[0] = temperaments[1];
        temperaments[1] = temp;
    }
    top2.sort((a,b) => b.lipsyncScore - a.lipsyncScore);
    console.log(top2[0].lipsyncScore + " - " + top2[0].getName());
    console.log(top2[1].lipsyncScore + " - " + top2[1].getName());
    let screen = new Scene();
    screen.clean();
    screen.createHeader("It's time...");
    ...
    screen.createImage(top2[0].image, "royalblue");
    screen.createBold(top2[0].getName() + ", you're a winner, baby!");
    //does not work with conjoined makeover! Do not try!!
    ...
    screen.createHorizontalLine();
    gdeChooseReasoning(top2[0].getName(), top2[0].lipstick.getName(), temperaments[0]);
    screen.createImage(top2[0].lipstick.image, "green");
    //remove the saved queen from the bottom
    bottomQueens.splice(bottomQueens.indexOf(top2[0].lipstick), 1);
    screen.createBold(top2[0].lipstick.getName() + ", as it is written, so it shall be done. You are safe.");
    top2[0].lipstick.addToTrackRecord("SAVED");
    top2[0].lipstick.unfavoritism += 2;
    top2[0].lipstick.ppe += 1.5;
    for (let i = 0; i < bottomQueens.length; i++)
        screen.createImage(bottomQueens[i].image, "tomato");
    screen.createBold("", "btm2");
    let btm2 = document.getElementById("btm2");
...
class Queen {
    ...
    getTributesElim() {
        this.performanceScore = this._calculateScores(25, 45, this._improvStat + this._lipsyncStat);
    }
    getTributesRemain() {
        this.performanceScore = this._calculateScores(25, 45, this._comedyStat + this._designStat);
    }
    getRusical() {
...
function CheckForReturning() {
    ...
    if (lalaparuza && (currentCast.length - eliminatedCast.length) >= 1 && (currentCast.length - eliminatedCast.length) < 3 && returningQueen == false && (all_stars || gde_all_stars)) {
        returningQueen = true;
        return true;
    }
    ...
    if (goldentributes && currentCast.length == Math.round((totalCastSize / 2)) && returningQueen == false && totalCastSize > 8) {
        console.log("RETURNING CHECK, PASSED");
        returningQueen = true;
        return true;
    }
    return false;
...
function returningQueenScreen() {
    ...
    if (goldentributes) {
        console.log("PREPARING TO LOAD CHALLENGE, PASSED");
        goldenTributes();
    }
    else {
        if (randomReturn)
...
let trackArray = [
    ...
    "Cool Death by The Oh Sees",
    "A Little Bit of Love by RuPaul",
    "Alone by Ty Segall",
...
function goldenTributes() {
    episodeCount++;
    for (var i = 0; i < currentCast.length; i++){
        currentCast[i].episodesOn++;
    }
    episodeChallenges.push("Golden Tributes");
    top2 = [];
    topQueens = [];
    bottomQueens = [];
    let screen = new Scene();
    ...
    let hostingQueens = currentCast.slice();
    let impersonatingQueens = []
    let usedIndicesIcons = [];
    let usedIndicesTracks = [];
    screen.createHeader("Golden Tributes!");
    screen.createBold("All of the eliminated All Stars get a chance to return to the competition by performing in the Golden Tributes set, a concert dedicated to the greatest icons of entertainment history!");
    screen.createParagraph("That is, those that Beatrice and Akane seem to like the most.");
    screen.createParagraph("The eliminated queens will impersonate legendary celebrities and perform to original remixes of iconic tracks! The remaining all-stars, on the other hand, will bring their design skills and comedy chops to the test by designing looks inspired by the celebrities' iconic styles AND hosting the event itself!");
    screen.createHorizontalLine();
    screen.createBold("The All Stars will now be assigned their musical inspirations, as well as the celebrities they will have to tribute.");
    console.log("INITIALIZING CHALLENGE, PASSED");
    for (let i = 0; i < eliminatedCast.length; i++){
        if (eliminatedCast[i].QueenDisqOrDept == true){
            eliminatedCast[i].addToTrackRecord("");
        } else { 
            impersonatingQueens.push(eliminatedCast[i]);
            screen.createImage(eliminatedCast[i].image);
            let queen = hostingQueens[randomNumber(0, hostingQueens.length - 1)]
            screen.createImage(queen.image);
            let newIconIndex = -1;
            let newTrackIndex = -1;
            while (newIconIndex == -1 || usedIndicesIcons.indexOf(newIconIndex) != -1) {
                newIconIndex = randomNumber(0, iconArray.length - 1);
            }
            usedIndicesIcons.push(newIconIndex);
            while (newTrackIndex == -1 || usedIndicesTracks.indexOf(newTrackIndex) != -1) {
                newTrackIndex = randomNumber(0, trackArray.length - 1);
            }
            usedIndicesTracks.push(newTrackIndex);
            screen.createBold(eliminatedCast[i].getName() + " and " + queen.getName() + " have been assigned " + iconArray[newIconIndex] + ". " + eliminatedCast[i].getName() + " will be performing to a remix of " + trackArray[newTrackIndex] + ".");
            eliminatedCast[i].getTributesElim();
            eliminatedCast[i].getRunway();
            queen.getTributesRemain();
            hostingQueens.splice(hostingQueens.indexOf(queen), 1);
        }
    }
    if (hostingQueens.length == 1) {
        let newIconIndex = -1;
        while (newIconIndex == -1 || usedIndicesIcons.indexOf(newIconIndex) != -1) {
            newIconIndex = randomNumber(0, iconArray.length - 1);
        }
        usedIndicesIcons.push(newIconIndex);
        screen.createImage(hostingQueens[0].image);
        screen.createBold(hostingQueens[0].getName() + " has been assigned " + iconArray[newIconIndex] + ".");
        hostingQueens[0].getTributesRemain();
    }
    console.log("QUEEN ASSIGNMENT, PASSED");
    screen.createHorizontalLine();
    sortPerformances(currentCast);
    screen.createBigText("Hosting queens' performances...");
    dipslayPerformanceRanking(currentCast);
    console.log("HOST PERFORMANCES, PASSED");
    comedyChallengeCounter++;
    designChallengeCounter++;
    screen.createHorizontalLine();
    sortPerformances(impersonatingQueens);
    screen.createBigText("Eliminated queens' performances...");
    let slay = impersonatingQueens.filter(function (queen) { return queen.performanceScore < 6; });
    let great = impersonatingQueens.filter(function (queen) { return queen.performanceScore >= 6 && queen.performanceScore < 16; });
    let good = impersonatingQueens.filter(function (queen) { return queen.performanceScore >= 16 && queen.performanceScore < 26; });
    let bad = impersonatingQueens.filter(function (queen) { return queen.performanceScore >= 26 && queen.performanceScore < 31; });
    let flop = impersonatingQueens.filter(function (queen) { return queen.performanceScore >= 31 && queen.performanceScore < 50; });
    createPerformanceDescWithID(slay, great, good, bad, flop, "2");
    console.log("IMPERSONATION PERFORMANCES, PASSED");
    screen.createHorizontalLine();
    screen.createBigText("Redemption Runway!");
    screen.createParagraph("The eliminated queens will bring it to the runway!");
    screen.createParagraph("The theme is... Redemption! The queens will re-do their most infamous runways from their original seasons!");
    slay = impersonatingQueens.filter(function (queen) { return queen.runwayScore < 6; });
    great = impersonatingQueens.filter(function (queen) { return queen.runwayScore >= 6 && queen.runwayScore < 16; });
    good = impersonatingQueens.filter(function (queen) { return queen.runwayScore >= 16 && queen.runwayScore < 26; });
    bad = impersonatingQueens.filter(function (queen) { return queen.runwayScore >= 26; });
    createRunwayDesc(slay, great, good, bad);
    console.log("RETURNING RUNWAY, PASSED");
    for (let i = 0; i < impersonatingQueens.length; i++) {
        impersonatingQueens[i].performanceScore -= (impersonatingQueens[i].runwayScore + impersonatingQueens[i].favoritism / 2);
    }
    impersonatingQueens.sort((a, b) => (a.performanceScore - b.performanceScore));
    currentCast.sort((a, b) => (a.performanceScore - b.performanceScore));
    topQueens.push(currentCast[0]);
    topQueens.push(currentCast[1]);
    let safeIndex = 2;
    ...
    for (let i = safeIndex; i < currentCast.length - 3; i++) {
        currentCast[i].addToTrackRecord("SAFE");
        currentCast[i].ppe += 3;
    }
    for (let i = 0; i < topQueens.length; i++) {
        topQueens[i].performanceScore += topQueens[i].favoritism;
    }
    topQueens.sort((a, b) => (a.performanceScore - b.performanceScore));
    for (let i = 0; i < 3; i++) {
        bottomQueens.push(currentCast[currentCast.length - 1 - i]);
    }
    topQueens.push(impersonatingQueens[0]);
    console.log("JUDGING, PASSED");
    screen.createHorizontalLine();
    screen.createButton("Proceed", "goldenTributesJudging()");
}
function goldenTributesJudging() {
    let screen = new Scene();
    window.scrollTo(0,0);
    screen.clean();
    screen.createHeader("Bring back my All Stars!");
    ...
    screen.createBold(top2[0].getName() + " condragulations, you're the Top All Star of the week!");
    let safeIndex = 1;
    console.log(topQueens.length - 1);
    for (let i = 0; i < topQueens.length - 1; i++) {
        screen.createImage(topQueens[i].image, "lightblue");
        safeIndex += 1;
    }
    ...
    screen.createParagraph("", "safes");
    let safes = document.querySelector("p#safes")
    for (let i = safeIndex; i < currentCast.length - 3; i++) {
        safes.innerHTML += currentCast[i].getName() + ", ";
    }
    if (currentCast.length - 4 == safeIndex) {
        safes.innerHTML += "you are safe.";
    }
    else if (currentCast.length - 3 != safeIndex) {
        safes.innerHTML += "you are all safe.";
    }
    screen.createHorizontalLine();
    for (let i = 0; i < bottomQueens.length; i++)
        screen.createImage(bottomQueens[i].image, "tomato");
    ...
    bottomQueens.splice(0, 1);
    screen.createHorizontalLine();
    top2.push(topQueens[topQueens.length - 1]);
    screen.createHeader("As for the returning queens...");
    screen.createBold("The other Top All Star of the week is...");
    screen.createImage(topQueens[topQueens.length - 1].image, "cyan");
    screen.createParagraph("..." + topQueens[topQueens.length - 1].getName() + "! Condragulations, my dear. As the winner of this challenge, you will have not one, but two chances to return to the competition!")
    for (let i = 0; i < eliminatedCast.length ; i++) {
        if (top2[1].getName() != eliminatedCast[i].getName() && !eliminatedCast[i].QueenDisqOrDept) {
            screen.createImage(eliminatedCast[i].image, "black");
            eliminatedCast[i].addToTrackRecord("OUT ");
        }
    }
    screen.createParagraph("My dears, I'm afraid it is not your time. Thank you for making this season of All Stars something truly unforgettable! Now, I must ask you all to sashay away once more...");
    screen.createHorizontalLine();
    screen.createBold("Now...");
    screen.createParagraph("First, the Top 2 All Stars of the week will lipsync against one another!");
    let temperaments = [];
    //decisionData stores the choice and the queen's temperament while making the choice
    let decisionData = whoToSave(top2[0], bottomQueens);
    temperaments.push(decisionData["temperament"]);
    top2[0].lipstick = decisionData["choice"];
    for (let o = 0; o < bottomQueens.length; o++) {
        if (top2[0].lipstick != bottomQueens[o]) {
            modRelation(2, 1, top2[0], bottomQueens[o]);
        }
            ...
            if (randomNumber(0, 1) == 0) {
                modRelation(2, 3, top2[0], bottomQueens[o]);
            }
        }
    }
    screen.createImage(top2[0].image, "cyan");
    screen.createImage(top2[0].lipstick.image, "green");
    screen.createImage(top2[1].image, "cyan");
    screen.createBold(top2[0].getName() + " chose to save " + top2[0].lipstick.getName(), "winV", "winVP");
    if (randomNumber(0, 1) == 0) {
        screen.createBold( top2[0].lipstick.getName() + " was very grateful.", "winV", "winVP");
    } else {
        screen.createBold( top2[0].lipstick.getName() + " was surprised, but glad nonetheless.","winV", "winVP");
    }
    let imageVoted = document.querySelectorAll("img[src='" + top2[0].lipstick.image +"']")[1];
    imageVoted.setAttribute("hidden", "hidden");
    mismovoto = false;
    ...
    let br = document.createElement("br");
    main.appendChild(br);
    screen.createButton("Show lipsticks", "showSingleVote()", "showvotes");
    screen.createButton("Proceed", "goldenTributesLipsync1(" + temperaments[0] + ")");
}
function goldenTributesLipsync1(saveTemperament) {
    //top2[0] - remaining queen, top2[1] - returning queen
    for (let i = 0; i < top2.length; i++) {
        top2[i].getASLipsync();
    }
    //will count this episode to the returning winner's track record
    top2[1].episodesOn++;
    console.log(top2[0].lipsyncScore + " - " + top2[0].getName());
    console.log(top2[1].lipsyncScore + " - " + top2[1].getName());
    let screen = new Scene();
    ...
    screen.createHorizontalLine();
    screen.createBold("Ladies, I've made my decision...");
    if (top2[0].lipsyncScore > top2[1].lipsyncScore) {
        top2[0].favoritism += 5;
        top2[0].ppe += 5;
        top2[0].addToTrackRecord("WIN");
        bottomQueens.push(top2[1]);
        screen.createImage(top2[0].image, "royalblue");
        screen.createBold(top2[0].getName() + ", you're a winner, baby!");
        screen.createImage(top2[1].image, "turquoise");
        screen.createBold(top2[1].getName() + ", thank you, my dear. Please head to the back of the stage and prepare to lip-sync for your life.");
        assasintable.push(top2[0].getName());
        assasinlipstick.push(top2[0].lipstick.getName());
        assasintable.push(top2[1].getName());
        assasinlipstick.push("N/A");
        screen.createHorizontalLine()
        gdeChooseReasoning(top2[0].getName(), top2[0].lipstick.getName(), saveTemperament);
        screen.createImage(top2[0].lipstick.image, "green");
        bottomQueens.splice(bottomQueens.indexOf(top2[0].lipstick), 1);
        screen.createBold(top2[0].lipstick.getName() + ", as it is written, so it shall be done. You are safe.");
        top2[0].lipstick.addToTrackRecord("SAVED");
        ...
        btm2.innerHTML += "This means the two of you will be lip-syncing for your lives! Will " + bottomQueens[0].getName() + " prevail? Or will " + bottomQueens[1].getName() + " take her place in the competition?";
        screen.createButton("Proceed", "goldenTributesLipsync2()");
    }
    else {
        top2[0].addToTrackRecord("WIN ");
        top2[0].favoritism += 4
        ...
        screen.createBold(top2[1].getName() + ", you're a winner, baby! Condragulations, you are now back in the competition!");
        assasintable.push(top2[1].getName());
        assasinlipstick.push("N/A");
        assasintable.push(top2[0].getName());
        assasinlipstick.push(top2[0].lipstick.getName());
        screen.createImage(top2[0].image, "cyan");
        screen.createParagraph(top2[0].getName() + ", great work this week. You are safe.");
        screen.createHorizontalLine();
        for (let i = 0; i < bottomQueens.length; i++)
        screen.createImage(bottomQueens[i].image, "tomato");
        screen.createBold("", "btm2");
        ...
        btm2.innerHTML += "I'm sorry my dears, but that means that BOTH of you will be lip-syncing for your lives tonight.";
        screen.createButton("Proceed", "lipSync()");
    }
}
function goldenTributesLipsync2() {
    //bottomQueens[0] - remaining queen, bottomQueens[1] - returning queen
    for (let i = 0; i < bottomQueens.length; i++) {
    ...
    screen.createHorizontalLine();
    screen.createBold("I've made my decision.");
    //doesn't count for total double shantays for greater season variety
    if (bottomQueens[0].lipsyncScore > 7 && bottomQueens[1].lipsyncScore > 7 && randomNumber(0, 100) <= 50 && !noDouble) {
        screen.createImage(bottomQueens[0].image, "magenta");
        screen.createImage(bottomQueens[1].image, "magenta");
        screen.createBold("Condragulations, shantay you both stay!!");
        bottomQueens[0].addToTrackRecord(" BTM2");
        bottomQueens[0].unfavoritism += 3;
        bottomQueens[0].ppe += 1;
        bottomQueens[1].addToTrackRecord("WIN   ");
        bottomQueens[1].favoritism += 4;
        bottomQueens[1].ppe += 5;
        screen.createBold("Condragulations " + bottomQueens[1].getName() + ", you are back in the competition!");
        eliminatedCast.splice(eliminatedCast.indexOf(bottomQueens[1]), 1);
        currentCast.push(bottomQueens[1]);
    }
    else if (bottomQueens[0].lipsyncScore > bottomQueens[1].lipsyncScore) {
        screen.createImage(bottomQueens[0].image, "tomato");
        screen.createBold(bottomQueens[0].getName() + ", shantay you stay.");
        bottomQueens[0].addToTrackRecord("BTM2");
        bottomQueens[0].unfavoritism += 3;
        bottomQueens[0].ppe += 1;
        bottomQueens[1].addToTrackRecord("WIN+OUT");
        bottomQueens[1].favoritism += 3;
        bottomQueens[1].ppe += 4;
        screen.createImage(bottomQueens[1].image, "crimson");
        screen.createBold("So close, and yet so far...");
        screen.createBold("You did a wonderful job this week, " + bottomQueens[1].getName() + ". Thank you for giving it your all this season. Now, sashay away...");
    }
    else {
        screen.createImage(bottomQueens[1].image, "lightcoral");
        screen.createBold(bottomQueens[1].getName() + ", shantay you stay. Condragulations, you are back in the competition!");
        bottomQueens[1].addToTrackRecord("WIN   ");
        bottomQueens[1].favoritism += 4;
        bottomQueens[1].ppe += 5;
        screen.createImage(bottomQueens[0].image, "red");
        if (chocolateBarTwist && !chocolateBarTwistCheck) {
            screen.createBold(bottomQueens[0].getName() + ", now your fate rests in the hands of the drag gods.");
            screen.createBold("If you have the golden chocolate bar, you will be safe.");
            if (chocolateBarCheck(bottomQueens[0]) == true) {
                screen.createImage("image/ChocolateBarWithTicket.webp", "gold");
                screen.createBold("You've got the GOLD BAR!!!! The gods have spoken!");
                screen.createBold(bottomQueens[0].getName() + "!! Condragulations, you are safe to slay another day!");
                bottomQueens[0].addToTrackRecord("CHOC");
...
class Scene {
    ...
    createButtonArgument(text, method, id = '', argumentArray) {
        let button = document.createElement("button");
        button.innerHTML = text;
        button.setAttribute("onclick", method + "(" + argumentArray + ")");
        button.setAttribute("id", id);
        this._MainBlock.appendChild(button);
    }
...
let gdeReasoningQueensBitch = [
    "they want to see a strong competitor eliminated.",
    "they wanted to give them a chance to redeem themselves in future challenges.",
    "they want the bottom lip-sync to be a SHOW!",
    "it's strategically the best choice."
]
let gdeReasoningQueensRandom = [
    "the decision was so difficult to make, they just followed their gut feeling.",
    "they thought it would make the competition more interesting",
    "it just seemed like the right decision to them.",
    "they felt like it."
]
let gdeReasoningQueensAnalytical = [
    "they wanted to save the person with the best track record.",
    "they wanted to make the fairest decision possible.",
    "they think that they deserve a second chance.",
    "they've had the fewest stumbles out of the bottom queens.",
    "they don't think it's their time to go yet."
]
let gdeReasoningQueensFair = [
    "they thought that they did the best out of the bottom 3.",
    "they tried to put themselves in Beatrice and Akane's shoes and chose fairly.",
    "they don't think they deserve to lip-sync this week.",
    "they don't think that they deserved to be in the bottom this week.",
    "they thought that they delivered a safe enough performance."
]
let gdeReasoningQueensBiased = [
    "they don't think that they deserved to be in the bottom this week.",
    "they don't think they deserve to lip-sync this week.",
    "they want to stay true to their relationship outside of the competition.",
    "they care about them and it seemed like the right thing to do.",
    "they consider them a friend, and also think they did a decent job in the challenge",
    "they are each other's support system and can't imagine being in the finale without each other.",
    "of their friendship on and off the show.",
    "they respect them and want to see them do well.",
    "they see a potential for an alliance between the two of them",
    "they tried to be fair while staying true to themselves.",
    "of their close relationship, as well as their performance in the challenge"
]
...
function gdeChooseReasoning(winQueen, savedQueen, temperament) {
    let screen = new Scene();
    let reasoning = -1
    switch (temperament) {
        case 0:
            reasoning = randomNumber(0, gdeReasoningQueensBitch.length - 1);
            screen.createBold(`${winQueen} chose ${savedQueen} because ${gdeReasoningQueensBitch[reasoning]}`);
            break;
        case 1:
            reasoning = randomNumber(0, gdeReasoningQueensRandom.length - 1);
            screen.createBold(`${winQueen} chose ${savedQueen} because ${gdeReasoningQueensRandom[reasoning]}`);
            break;
        case 2:
            reasoning = randomNumber(0, gdeReasoningQueensAnalytical.length - 1);
            screen.createBold(`${winQueen} chose ${savedQueen} because ${gdeReasoningQueensAnalytical[reasoning]}`);
            break;
        case 3:
            reasoning = randomNumber(0, gdeReasoningQueensFair.length - 1);
            screen.createBold(`${winQueen} chose ${savedQueen} because ${gdeReasoningQueensFair[reasoning]}`);
            break;
        case 4:
            reasoning = randomNumber(0, gdeReasoningQueensBiased.length - 1);
            screen.createBold(`${winQueen} chose ${savedQueen} because ${gdeReasoningQueensBiased[reasoning]}`);
            break;
    }
}
//2 queens events
let twoQueensRelation1 = [
    ...
    "are sharing wigs.",
    "kiss eachother.",
    "are gossiping about their fellow competitors.",
...
let twoQueensRelation2 = [
    ...
    "are having a heartfelt moment when talking about their families.",
    ...
    "are finding out that they have a ton in common."
];
...
let threeQueensRelation3 = [
    "get into a heated debate about the progress of the drag scene.",
    "are talking about who they think should go home.",
    "have a complex discussion about gender."
];
...
let fourQueensRelation3 = [
    "think that they are the best ones in here.",
...
function whoToSave(lsWinner, btmQueens) {
    let btmQueensCopy = btmQueens.slice();
    let temperamentRNG = randomNumber(1, 100);
    let temperament = -1;
    //competitive, will save the btmQueen, who would have the lowest finale score
    if (temperamentRNG <= 20) {
        temperament = 0;
        console.log("BITCH");
        shuffle(btmQueensCopy);
        btmQueensCopy.sort((a,b) => (a.favoritism - a.unfavoritism) - (b.favoritism - b.unfavoritism));
    }
    //unpredictable, will select a random btmQueen
    else if (temperamentRNG <= 25) {
        temperament = 1;
        console.log("RANDOM");
        shuffle(btmQueensCopy);
    }
    //analytical, will select the queen with the highest finale score. In case of a tie, it will select the queen with a better performance in the challenge.
    else if (temperamentRNG <= 50) {
        temperament = 2;
        console.log("ANALYTICAL");
        shuffle(btmQueensCopy);
        btmQueensCopy.sort((a,b) => {
            if ((b.favoritism - b.unfavoritism) - (a.favoritism - a.unfavoritism) == 0) {
                return a.performanceScore - b.performanceScore
            }
            return (b.favoritism - b.unfavoritism) - (a.favoritism - a.unfavoritism) 
        });
    }
    //fair, will select the queen with the best performance in the challenge and runway
    else if (temperamentRNG <= 60) {
        temperament = 3;
        console.log("FAIR");
    }
    //fair, but biased, will select the queen with the closest relationship type. In case of a tie, will select the queen with the best challenge performance
    else {
        temperament = 4;
        console.log("BIASED");
        let bestQueens = [];
        //Limit to friends. If not friends, neutrals, if all are enemies, base it off theh challenge
        for (let i = 0; i < btmQueensCopy.length; i++) {
            if (isFriend(btmQueensCopy[i], lsWinner)) {
                bestQueens.push(btmQueensCopy[i]);
            }
        }
        if (bestQueens.length == 0) {
            for (let i = 0; i < btmQueensCopy.length; i++) {
                if (isEnemy(btmQueensCopy[i], lsWinner)) {
                    continue;
                }
                bestQueens.push(btmQueensCopy[i]);
            }
            if (bestQueens.length == 0) {
                bestQueens = btmQueensCopy.slice();
            }
        }
        return {
